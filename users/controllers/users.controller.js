...
exports.insert = (req, res) => {
    ...
    if(/^([a-zA-Z]+([ /]?[a-zA-Z]+)*)+S/.test(req.body.lastName) === true){
    ...
    if(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,6}$/.test(req.body.email) === true){
    ...
    if(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[-!$%^&*()_+|~=`{}\[\]:";'<>?,.\/]).{8,50}$/.test(req.body.password) === true){
    ...
    if(/^([89][0-9]{7})$/.test(req.body.phoneNo) === true){
